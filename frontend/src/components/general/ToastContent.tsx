...
export const ToastContent = ({ title, message, variant = "default", icon, toastId, isVisible = true }: ToastContentProps) => {
  const lines = message
    .split("\n")
    .map(line => line.trim())
    .filter(Boolean)
  ...
  return (
    <div className={`app-toast app-toast--${variant} ${isVisible ? "app-toast--enter" : "app-toast--leave"}`} dir="rtl">
      <button type="button" className="app-toast__close" aria-label="סגירת הודעה" onClick={() => toastId && toast.dismiss(toastId)}>
        ×
      </button>
      <div className="app-toast__layout">
        <div className="app-toast__icon-container">
          <div className="app-toast__icon-box">{icon ?? defaultIcon}</div>
        </div>
